...
public class PaymentAPI {
	...
	@Value("Payment.TRANSACTION_FAILED_CVV_NOT_MATCHING")
	private String  cvvNotMatching;
	
	@Value("PaymentAPI.PAYMENT_FAILURE_FALLBACK")
	...
	public ResponseEntity<List<CardDTO>> getCardsOfCustomer(@PathVariable String customerEmailId,
		...
		System.out.println("HERE" + customerEmailId + " " + cardType);
		List<CardDTO> listOfCards=paymentService.getCustomerCardOfCardType(customerEmailId, cardType);

		return new ResponseEntity<List<CardDTO>>(listOfCards,HttpStatus.OK);
	}

	@PostMapping(value = "/customer/{customerEmailId}/pay-order")

	// 1- Annotate this method for handling resilience: Completed
	// 2- Get the order details from CustomerMS for the given orderId (available in
	// 	  TransactionDTO) : done
	// 3- Update the Transaction details with the obtained Order details in above step,
	// 	  along with transaction date and total price : done
	// 4- Authenticate the transaction details for the given customer by calling
	// 	  authenticatePayment() method of PaymentService and assign the return value to transactionDTO
	// 5- Add the transaction details to the database by calling addTransaction()
	// 	  method of PaymentService 
	// 6- Update the order status by calling updateOrderAfterPayment() method of
    // 	  PaymentCircuitBreakerService by passing transaction status as TRANSACTION_SUCCESS
    // 7- Set the appropriate success or failure message and return the same
	...
	public ResponseEntity<String> payForOrder(
		...
		ResponseEntity<OrderDTO> order=template.getForEntity("http://CustomerMS/Ekart/customerorder-api/order/"+orderId, OrderDTO.class);
		OrderDTO orderDTO= order.getBody();
		
		//updating the order details in transaction dto
		...
		int transactionId = paymentService.addTransaction(transactionDTO);
		
		//updating  order after payment
		paymentCircuitBreakerService.updateOrderAfterPayment(orderId,"TRANSACTION_SUCCESS");
		
		//success message
		String succesMessage=
				environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_ONE")+transactionDTO.getTotalPrice()+" "
				+environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_TWO")+orderId
				+environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_THREE")+transactionId;
		
		return new ResponseEntity<String>(succesMessage,HttpStatus.OK);
		
	}

	// 1- Implement a fallback method here
	// 2- If exception message is Payment.TRANSACTION_FAILED_CVV_NOT_MATCHING then set
	// 	  message as Payment.TRANSACTION_FAILED_CVV_NOT_MATCHING
	// 3- Else set the message as PaymentAPI.PAYMENT_FAILURE_FALLBACK
	...
	public ResponseEntity<String> payForOrderFallback(String customerEmailId, TransactionDTO transactionDTO,
		...
		// Write your logic here
		if(exception.getMessage().equals(this.cvvNotMatching))
			return new ResponseEntity<String>(environment.getProperty(this.cvvNotMatching),HttpStatus.BAD_REQUEST);
		
		return new ResponseEntity<String>(environment.getProperty(this.paymentFailure),HttpStatus.BAD_REQUEST);
	}
}
